...
const getTransport = async botId => {
  ...
  if (!config || !config.transportConnectionString || config.transportConnectionString === '<<changeme>>') {
    throw new Error(
      `You should configure the "basic-skills" module with valid SMTP credentials to send emails. Please see 'global/config/basic-skills.json'.`
    )
  }
